...
var pass_code = form.elements['pass_code']
...
body.addEventListener('submit', function(ev) {
    ...
    invite(channel ? channel.value : null, coc && coc.checked ? 1 : 0, email.value, gcaptcha_response.value, pass_code.value, function(err, msg) {
        if (err) {
...
function invite(channel, coc, email, gcaptcha_response_value, pass_code, fn) {
    request
