...
export default function splash({ path, name, org, coc, logo, active, total, channels, large, iframe, gcaptcha_sitekey }) {
    let div = dom('.splash', !iframe && dom('.logos',
        ...
        dom('p.status',
            active ? [
                dom('b.active', active), ' 人がオンライン ',
                dom('b.total', total), ' 人が登録'
            ] : [dom('b.total', total), ' 人が登録']
        ),
        dom('form id=invite',
            channels && (
            ...
            dom('input.form-item type=email name=email placeholder=メールアドレス ' +
                (!iframe ? 'autofocus' : '')),
            dom('input.form-item type=text name=pass_code placeholder=パスコード '),
            dom('br'),
