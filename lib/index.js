...
export default function slackin({
    ...
    if (!pass_code) throw new Error('Must provide an `pass_code`.')
    ...
    app.get('/', (req, res) => {
        ...
        let page = dom('html',
            ...
            splash({ coc, path, css, name, org, logo, channels, active, total, gcaptcha_sitekey, pass_code })
        )
        res.type('html')
    ...
    app.post('/invite', json(), (req, res, next) => {
        ...
        let send_pass_code = req.body.send_pass_code;
        ...
        if (send_pass_code != pass_code) {
            return res.status(400).json({ msg: 'パスコードが違います' })
        }
